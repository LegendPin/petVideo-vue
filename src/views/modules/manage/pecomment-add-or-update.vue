    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/manage/pecomment/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.petId = data.pecomment.petId
                this.dataForm.commentUser = data.pecomment.commentUser
                this.dataForm.commentName = data.pecomment.commentName
                this.dataForm.commentPic = data.pecomment.commentPic
                this.dataForm.content = data.pecomment.content
                this.dataForm.createTime = data.pecomment.createTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/manage/pecomment/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
