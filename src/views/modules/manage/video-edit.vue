    ...
    <div class="vp-main">
            ...
            <el-row>
                ...
                <el-col :span="5" :offset="5">
                    <el-upload
                        ...
                        <img v-else-if="videoInfo.filePic" :src="$http.adornUrl(videoInfo.filePic)" class="avatar">
        ...
        <el-card class="box-card" style="width: 50%;left: 100px;position: relative">
            <div slot="header" class="clearfix">
                <span>当前文件：</span>
                <a @click="getStatic(videoInfo.fileUrl) ">{{videoInfo.fileName}}</a>
            </div>
            <el-upload
                ref="upload-video"
                :action="uploadVideo()"
        ...
        <el-button type="primary" size="large" @click="publicVideo()" class="vp-btn-sumbmit">更新视频</el-button>
    </div>
</template>
            ...
            getVideoInfo(){
                this.$http({
                    ...
                    if (data && data.code === 0) {
                        ...
                        this.dataForm.name = this.videoInfo.name;
                        this.dataForm.type = this.videoInfo.type;
                        this.dataForm.fileName = this.videoInfo.fileName;
                        this.dataForm.fileUrl = this.videoInfo.fileUrl;
                        this.dataForm.filePic = this.videoInfo.filePic;
                        this.dataForm.remark = this.videoInfo.remark;
                    } else {
                        this.$message.error(data.msg)
                    }
                })
            ...
            publicVideo(){
                ...
                this.$refs['dataForm'].validate((valid) => {
                    if (valid){
                        ...
                        this.$http({
                            url: this.$http.adornUrl('/manage/videoinfo/update'),
                            method: 'post',
